...
export async function getDistanceByZip(zip1, zip2) {
    ...
    try {
      const [coord1, coord2] = await Promise.all([getCoordinatesByZip(zip1), getCoordinatesByZip(zip2)]);
      ...
      console.error("Error calculating distance between zip codes:", zip1, zip2, error);
      return Infinity;
    }
  }
