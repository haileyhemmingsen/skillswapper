...
function Post({ post, onArchiveToggle, editPost }) {
  ...
  return (
    <div
      className={`${styles.postContainer} ${post.archive ? styles.archivedPost : ''}`}
    >
      <div className={styles.postHeader}>
        <div className={styles.userInfo}>
          ...
          />
          <span
            className={`${styles.username} ${post.archive ? styles.archivedText : ''}`}
          >
            {post.username}
          </span>
        </div>
        <div className={styles.postMeta}>
          <span
            className={`${styles.postDate} ${post.archive ? styles.archivedText : ''}`}
          >
      ...
      <div className={styles.postBody}>
        <div className={styles.contentLine}>
          <span
            className={`${styles.contentText} ${post.archive ? styles.archivedText : ''}`}
          >
            {seeking}
          </span>
          <img
    ...
    const fetchMyPosts = async () => {
      try {
        const response = await axios.get(
          'http://localhost:3080/api/v0/getUserPosts',
          {
  ...
  return (
    <div className={styles.pageContainer}>
      <div className={styles.profileSection}>
        ...
        <img src={userImage} alt="Profile" className={styles.profileAvatar} />
